...
struct MaxHeap
{
    ...
    {   
        int * copy_of_array = new int[maxSize];
        for (int iter = 0; iter <= size; iter++) {
            copy_of_array[iter] = array[iter];
        }
        int copy_of_size = size;

        int * temp = new int[size];
        for (int iter = 0; iter <= size; iter++) {
            temp[size-iter] = ExtractMax();
        }

        this->array = copy_of_array;
        this->size = copy_of_size;
...
int main()
{
    ...
    heap->HeapSort(heap->array, heap->size);
